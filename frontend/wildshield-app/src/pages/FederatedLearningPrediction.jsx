...
import { MdCloudUpload, MdPets, MdCameraAlt, MdAutoGraph } from 'react-icons/md';
import { predictImage } from '../services/api';

function FederatedLearningPrediction() {
    ...
    return (
        <div className="fade-in">
            <div className="page-header" style={{ marginBottom: '32px' }}>
                <h2 style={{ fontSize: '24px', marginBottom: '8px', color: '#fff' }}>Federated Learning Prediction</h2>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--accent-purple)', fontSize: '14px', marginBottom: '12px' }}>
                    <MdAutoGraph />
                    <span style={{ fontWeight: 600 }}>Decentralized Intelligence Mode</span>
                </div>
                <p style={{ color: 'var(--text-muted)', fontSize: '15px', lineHeight: 1.6, maxWidth: '800px' }}>
                    Decentralized wildlife classification using federated learning models. 
                    This module performs wildlife classification using a federated learning approach, 
                    ensuring that sensitive data remains on edge devices while only model updates are shared.
                </p>
            </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
                            ...
                            {file && (
                                <button onClick={handlePredict} disabled={loading} style={{ padding: '8px 24px', background: 'var(--accent-purple)', color: '#fff', border: 'none', borderRadius: '6px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', boxShadow: '0 0 15px rgba(168, 85, 247, 0.4)', opacity: loading ? 0.7 : 1 }}>
                                    {loading ? 'Analyzing...' : 'Start Federated Inference'}
                                </button>
                            )}
                        </div>
                    </div>
...
export default FederatedLearningPrediction;
